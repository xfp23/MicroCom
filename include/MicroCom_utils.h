...
#define MICROCOM_CHECK_CAN_CHANNEL(ch) \
    do { if ((ch) >= MICROCOM_CAN_CHANNEL_NUM) { return MICROCOM_CHANNEL_ERR; } } while (0)

#define MICROCOM_CHECK_CYCLE_CAN_SIZE(size) \
    do { if ((size) == 0u || (size) > MICROCOM_CAN_CYCLEMSG_SIZE) { return MICROCOM_PARAM_INVALID; } } while (0)

#define MICROCOM_CHECK_EVENT_CAN_SIZE(size) \
    do { if ((size) == 0u || (size) > MICROCOM_CAN_EVENTMSG_SIZE) { return MICROCOM_PARAM_INVALID; } } while (0)
